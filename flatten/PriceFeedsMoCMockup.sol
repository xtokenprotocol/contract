...
// Dependency file: contracts/feeds/mockup/PriceFeedsMoC.sol
...
contract PriceFeedsMoC is IPriceFeedsExt, Ownable {
    ...
    address public tfiOracleAddress;

    event SetMoCOracleAddress(address indexed mocOracleAddress, address changerAddress);
    event SetTFIOracleAddress(address indexed tfiOracleAddress, address changerAddress);

    constructor(
        address _mocOracleAddress,
        address _tfiOracleAddress)
        public
    {
        setMoCOracleAddress(_mocOracleAddress);
        setTFIOracleAddress(_tfiOracleAddress);
    }
        ...
        returns (uint256)
    {
        ...
        if (hasValue) {
            ...
            (uint256 price,) = ITFIOracle(tfiOracleAddress).getPricing();
            return price;  
        }
    }
    ...
    function setTFIOracleAddress(
        address _tfiOracleAddress) 
        public 
        onlyOwner 
    {
        require(Address.isContract(_tfiOracleAddress), "_tfiOracleAddress not a contract");
        tfiOracleAddress = _tfiOracleAddress;
        emit SetTFIOracleAddress(tfiOracleAddress, msg.sender);
    }
}


// Root file: contracts/feeds/mockup/PriceFeedsMoCMockup.sol

pragma solidity 0.5.17;

// import "contracts/feeds/mockup/PriceFeedsMoC.sol";
