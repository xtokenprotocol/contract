...
// Root file: contracts/modules/SwapsExternal.sol
        ...
        returns (uint256 destTokenAmountReceived, uint256 sourceTokenAmountUsed)
    {
        ...
        if (msg.value != 0) {
            if (sourceToken == address(0)) {
                sourceToken = address(wbaseToken);
            }
            require(sourceToken == address(wbaseToken), "sourceToken mismatch");
            require(msg.value == sourceTokenAmount, "sourceTokenAmount mismatch");
            wbaseToken.deposit.value(sourceTokenAmount)();
        } else {
