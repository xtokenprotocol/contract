...
contract LoanTokenSettingsLowerAdmin is AdvancedToken {
    ...
    // It is // important to maintain the variables order so the delegate calls can access thaifiContractAddress
        ...
        onlyAdmin
    {
        ...
        loanParamsIdList = ProtocolSettingsLike(thaifiContractAddress).setupLoanParams(loanParamsList);
        ...
        onlyAdmin
    {
        ...
        ProtocolSettingsLike(thaifiContractAddress).disableLoanParams(loanParamsIdList);
    }
...
// Root file: contracts/connectors/loantoken/LoanTokenLogicStandard.sol
    ...
    */
        ...
        returns (bool)
    {
        return _internalTransferFrom(
            ...
            _to,
            _value,
            ProtocolLike(thaifiContractAddress).isLoanPool(msg.sender) ?
                uint256(-1) :
    ...
    function totalAssetBorrow()
        ...
        returns (uint256)
    {
        return ProtocolLike(thaifiContractAddress).getTotalPrincipal(
            address(this),
            loanTokenAddress
        );
    }
        ...
        returns (uint256 principal, uint256 collateral, uint256 interestRate)
    {
        ...
        collateral = ProtocolLike(thaifiContractAddress).getEstimatedMarginExposure(
            loanTokenAddress,
            collateralTokenAddress,
            loanTokenSent,
            collateralTokenSent,
        ...
        returns (uint256 depositAmount)
    {
        if (borrowAmount != 0) {
            ...
            if (newBorrowAmount <= _underlyingBalance()) {
                return ProtocolLike(thaifiContractAddress).getRequiredCollateral(
                    loanTokenAddress,
    ...
    {
        if (depositAmount != 0) {
            borrowAmount = ProtocolLike(thaifiContractAddress).getBorrowAmount(
                loanTokenAddress,
                collateralTokenAddress != address(0) ? collateralTokenAddress : wbaseTokenAddress,
                depositAmount,
                50 * 10**18, // initialMargin,
        ...
        internal
    {
        ...
        if (lastSettleTime_ != ts) {
            ProtocolLike(thaifiContractAddress).withdrawAccruedInterest(
                loanTokenAddress
            );
        ...
        returns (uint256 totalDeposit)
    {
        ...
        if (collateralTokenSent != 0) {
            ...
            (uint256 collateralToLoanRate, uint256 collateralToLoanPrecision) = FeedsLike(ProtocolLike(thaifiContractAddress).priceFeeds()).queryRate(
                collateralTokenAddress,
                loanTokenAddress
            );
            ...
            uint256 collateralTokenAmount = ProtocolLike(thaifiContractAddress).getSwapExpectedReturn(
                loanTokenAddress,
                collateralTokenAddress,
                loanTokenAmount
            );
        ...
        returns (uint256, uint256)
    {
        ...
        (sentAmounts[1], sentAmounts[4]) = ProtocolLike(thaifiContractAddress).borrowOrTradeFromPool.value(msgValue)( // newPrincipal, newCollateral
            loanParamsId,
            loanId,
            withdrawAmount != 0 ? // isTorqueLoan
                true :
        ...
        returns (uint256 msgValue)
    {
        ...
        if (withdrawalAmount != 0) { // withdrawOnOpen == true
            ...
            if (newPrincipal > withdrawalAmount) {
                _safeTransfer(_loanTokenAddress, thaifiContractAddress, newPrincipal - withdrawalAmount, "");
            }
        } else {
            _safeTransfer(_loanTokenAddress, thaifiContractAddress, newPrincipal, "27");
        }
        ...
        if (collateralTokenSent != 0) {
            if (collateralTokenAddress == _wbaseToken && msgValue != 0 && msgValue >= collateralTokenSent) {
                ...
                _safeTransfer(collateralTokenAddress, thaifiContractAddress, collateralTokenSent, "28-a");
                msgValue -= collateralTokenSent;
            } else {
                _safeTransferFrom(collateralTokenAddress, msg.sender, thaifiContractAddress, collateralTokenSent, "28-b");
            }
        }

        if (loanTokenSent != 0) {
            _safeTransferFrom(_loanTokenAddress, msg.sender, thaifiContractAddress, loanTokenSent, "29");
        }
    }
        ...
        returns (uint256)
    {
        if (assetBorrow != 0 && assetSupply >= assetBorrow) {
            return _avgBorrowInterestRate(assetBorrow)
                ...
                .mul(SafeMath.sub(10**20, ProtocolLike(thaifiContractAddress).lendingFeePercent()))
                .div(10**40);
        }
    }
            ...
            uint256 interestUnPaid)
    {
        ...
        uint256 interestFeePercent;
        (,,interestOwedPerDay,interestUnPaid,interestFeePercent,) = ProtocolLike(thaifiContractAddress).getLenderInterestData(
            address(this),
            loanTokenAddress
        );
